...
class ProductModel {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'weight': weight,
      'dimensions': dimensions, // Already a map
      // 'brands': brands?.map((b) => b.toMap())?.toList(), // Assuming BrandModel has toMap()
      // 'categories': categories?.map((c) => c.toMap())?.toList(),
      'tags': tags,
      'images': images,
      'image': image,
      // 'attributes': attributes?.map((a) => a.toMap())?.toList(),
      // 'defaultAttributes': defaultAttributes?.map((a) => a.toMap())?.toList(),
