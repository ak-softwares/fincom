...
class NewOrderFoundController extends GetxController{
  ...
  void selectAll() {
    selectedOrders.addAll(parentOrders.map((order) => order.orderId ?? 0));
  }
  ...
  void deleteSelectedOrders() {
    parentOrders.removeWhere((order) => selectedOrders.contains(order.orderId));
  ...
  Future<void> uploadSelectedOrders() async {
    try {
      ...
      List<OrderModel> selectedOrderList = parentOrders.where((order) {
        return selectedOrders.contains(order.orderId);
      }).toList();
      ...
      AppMassages.errorSnackBar(title: 'Error in upload', message: e.toString());
    } finally {
      isUploading(false);
    }
  }
    ...
    try {
      ...
      while (true) {
        ...
        List<OrderModel> filteredOrders = fetchedOrders.where((order) {
          return !uploadedOrdersIds.contains(order.orderId);
        }).toList();
