...
class TransactionController extends GetxController {
  ...
  Future<void> refreshTransactions() async {
    try {
      ...
      TLoaders.errorSnackBar(title: 'Error in Transactions getting', message: e.toString());
    } finally {
      isLoading(false);
    }
  }
  ...
  Future<void> getTransactionByEntity({required EntityType entityType, required int entityId}) async {
    try {
      ...
      TLoaders.errorSnackBar(title: 'Error in vendors transactions', message: e.toString());
    }
  }
  ...
  Future<TransactionModel> getTransactionByPurchaseId({required int purchaseId}) async {
    try {
      ...
      TLoaders.errorSnackBar(title: 'Error in purchase transactions', message: e.toString());
  ...
  void saveTransaction() {
    TransactionModel transaction = TransactionModel(
      ...
      toEntityId: selectedVendor.value.vendorId,
  ...
  Future<void> addTransaction({required TransactionModel transaction}) async {
    try {
      TFullScreenLoader.openLoadingDialog('Updating your transaction...', Images.docerAnimation);
      ...
      TFullScreenLoader.stopLoading();
      TLoaders.customToast(message: 'Transaction uploaded successfully!');
  ...
  void saveUpdatedTransaction({required TransactionModel previousTransaction}) {
    ...
    TransactionModel transaction = TransactionModel(
      ...
      toEntityId: selectedVendor.value.vendorId ?? previousTransaction.toEntityId,
  ...
  Future<void> updateTransaction({required TransactionModel transaction, required TransactionModel previousTransaction}) async {
    try {
      TFullScreenLoader.openLoadingDialog('Updating transaction...', Images.docerAnimation);
      ...
      TFullScreenLoader.stopLoading();
      TLoaders.customToast(message: 'Transaction updated successfully!');
      Navigator.of(Get.context!).pop();
    } catch (e) {
      TFullScreenLoader.stopLoading();
  ...
  Future<TransactionModel> getTransactionByID({required String id}) async {
    try {
      ...
      TLoaders.errorSnackBar(title: 'Error fetching transaction', message: e.toString());
      return TransactionModel();
    }
  }
