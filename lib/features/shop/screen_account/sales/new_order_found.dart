...
import '../../controller_account/sales_controller/new_order_found_controller.dart';
                  ...
                  return GridLayout(
                    ...
                    itemBuilder: (context, index) {
                      if (index < orders.length) {
                        ...
                        return Obx(() => InkWell(
                              onTap: () => newOrderFoundController.toggleSelection(order.id ?? 0),
                              child: Stack(
                                children: [
                                  SingleOrderTile(order: order),
                                  if (newOrderFoundController.selectedOrders.contains(order.id))
                                    Container(
                                      height: double.infinity,
                                      decoration: BoxDecoration(
                                        color: newOrderFoundController.selectedOrders.contains(order.id)
                                            ? Colors.blue.withOpacity(0.5)
                                            : Colors.white,
                                        border: Border.all(color: newOrderFoundController.selectedOrders.contains(order.id)
                                            ? Colors.blue
                                            : Colors.grey
                                        ),
                                        borderRadius: BorderRadius.circular(10),
